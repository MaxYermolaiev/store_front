...
class Engine {
    ...
    request = async (url, method = "GET", body = null, headers = {}) => {
        ...
        if(this.#token&&this.#_id){headers['x-token'] = this.#token}

        if (body !== null) {
            body = this.toJSON(body);
            headers['Content-type'] = 'application/json';
        }
        console.log(`${this.end_point}/${url}`)
        let response = await fetch(`${this.end_point}/${url}`, {method, body, headers});

        if (!response.ok) {
            const {message,errors=null} = await response.json();
            const {status} = response;
            throw new ApiError(this.mode === "dev" ? `Response status ${status}, error message - ${message}` : `Errors occurred - ${message}`,status, errors);
        }
    ...
    testString=(str)=> /([A-Za-zА-Яа-я]){3,32}/.test(str);
